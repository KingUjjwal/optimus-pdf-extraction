...
use optimus_core::{generate_ascii_grid, TextSpan};
...
pub async fn discover_schema_command(
    spans_json: String,
    ...
    let schema = if let Some(provider) = provider {
        tracing::info!(
            "Using LLM provider {} for schema inference",
            provider.model_name()
        );
        ...
        .await
        {
            Ok(Ok((s, usage))) => {
                tracing::info!(
                    "LLM schema inference succeeded: {} chars, input={}, output={}",
                    s.len(),
                    usage.input_tokens,
                    usage.output_tokens
                );
            ...
            Ok(Err(e)) => {
                tracing::warn!(
                    "LLM schema inference failed: {}, falling back to heuristic",
                    e
                );
