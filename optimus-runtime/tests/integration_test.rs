...
use optimus_runtime::{ExtractedRecord, WasmHost, build_arrow_record_batch, process_pdfs_parallel};
use optimus_core::{extract_spans_from_bytes, build_spatial_graph};
use optimus_agent::{serialize_flat_graph, discover_schema_from_spans, compile_extraction_logic};
...
fn test_full_pipeline_from_bytes() {
    ...
    let wasm_bytes = compile_extraction_logic(&layout_id, &graph, &schema, cache_dir.path()).unwrap();
    assert!(!wasm_bytes.is_empty());

    let host = WasmHost::new();
    let json_output = host.execute_extraction(&layout_id, &wasm_bytes, &flat_graph).unwrap();
...
fn test_process_pdfs_parallel_single() {
    ...
    assert!(pdf_path.exists(), "PDF file should exist at {}", pdf_path.display());
    ...
    assert!(!spans.is_empty(), "extract_spans should return spans (real or mock)");
    ...
    let flat_graph = serialize_flat_graph(&graph);
    let host = WasmHost::new();
    let json = host.execute_extraction(&layout_id, &wasm_bytes, &flat_graph)
...
fn test_cache_hit_skips_recompilation() {
    ...
    let spans1 = vec![
        optimus_core::TextSpan { text: "INVOICE".into(), x0: 50.0, y0: 750.0, x1: 150.0, y1: 770.0 },
        optimus_core::TextSpan { text: "Invoice Number:".into(), x0: 50.0, y0: 700.0, x1: 150.0, y1: 715.0 },
        optimus_core::TextSpan { text: "INV-X-001".into(), x0: 180.0, y0: 700.0, x1: 260.0, y1: 715.0 },
        optimus_core::TextSpan { text: "Date:".into(), x0: 50.0, y0: 680.0, x1: 100.0, y1: 695.0 },
        optimus_core::TextSpan { text: "2026-01-01".into(), x0: 180.0, y0: 680.0, x1: 270.0, y1: 695.0 },
        optimus_core::TextSpan { text: "Total:".into(), x0: 400.0, y0: 400.0, x1: 450.0, y1: 415.0 },
        optimus_core::TextSpan { text: "$999.00".into(), x0: 500.0, y0: 400.0, x1: 560.0, y1: 415.0 },
    ];
    ...
    let spans2 = vec![
        optimus_core::TextSpan { text: "INVOICE".into(), x0: 50.0, y0: 750.0, x1: 150.0, y1: 770.0 },
        optimus_core::TextSpan { text: "Invoice Number:".into(), x0: 50.0, y0: 700.0, x1: 150.0, y1: 715.0 },
        optimus_core::TextSpan { text: "INV-Y-002".into(), x0: 180.0, y0: 700.0, x1: 260.0, y1: 715.0 },
        optimus_core::TextSpan { text: "Date:".into(), x0: 50.0, y0: 680.0, x1: 100.0, y1: 695.0 },
        optimus_core::TextSpan { text: "2026-06-15".into(), x0: 180.0, y0: 680.0, x1: 270.0, y1: 695.0 },
        optimus_core::TextSpan { text: "Total:".into(), x0: 400.0, y0: 400.0, x1: 450.0, y1: 415.0 },
        optimus_core::TextSpan { text: "$150.75".into(), x0: 500.0, y0: 400.0, x1: 560.0, y1: 415.0 },
    ];
    let layout_id2 = calculate_layout_id(&spans2);
    assert_eq!(layout_id1, layout_id2, "layout IDs must match for same anchor positions");
    ...
    let json2 = host.execute_extraction(&layout_id2, &wasm1, &flat2).unwrap();
    let record2: ExtractedRecord = serde_json::from_str(&json2).unwrap();
    assert_eq!(record2.fields.get("invoice_number").map(|s| s.as_str()), Some("INV-Y-002"));
    assert_eq!(record2.fields.get("date").map(|s| s.as_str()), Some("2026-06-15"));
    assert_eq!(record2.fields.get("total").map(|s| s.as_str()), Some("$150.75"));
}
...
fn test_error_propagation_bad_input() {
    ...
    let msg = format!("{}", err);
    assert!(msg.contains("Failed to open PDF"), "expected PdfOpenFailed, got: {}", msg);
}
