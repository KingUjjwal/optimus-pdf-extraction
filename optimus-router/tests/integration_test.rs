...
fn test_layout_id_stability() {
    ...
    assert_eq!(hashes.len(), 1, "Layout ID is not stable! Expected 1 hash, found {}", hashes.len());
}
...
fn test_layout_id_distinguishes_templates() {
    ...
    assert_ne!(hash1, hash2, "Different templates must produce different hashes");
}
