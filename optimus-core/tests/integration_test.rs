use optimus_core::{
    build_spatial_graph, extract_spans, generate_ascii_grid_with_config,
    GridConfig, GridFormat, TextSpan
};
...
fn test_real_pdf_report() {
    ...
    // Verify text exists
    let title = graph.nodes.iter().find(|n| n.span.text.contains("Earnings Report"));
...
fn test_real_pdf_form() {
    ...
    let fn_node = graph.nodes.iter().find(|n| n.span.text.contains("First Name:"));
    assert!(fn_node.is_some(), "First Name: not found in graph");
    
    // Nearest right should be John
    let fn_node = fn_node.unwrap();
    assert!(fn_node.nearest_right.is_some(), "Expected right neighbor for First Name:");
    let right_neighbor = fn_node.nearest_right.as_ref().unwrap();
    assert!(right_neighbor.text.contains("John"), "Expected 'John', got {}", right_neighbor.text);
...
fn test_rtree_performance_1000_spans() {
    ...
    assert!(duration.as_millis() < 150, "R-tree query performance took {}ms, expected < 150ms", duration.as_millis());
}
